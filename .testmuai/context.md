...
- Product Detail: `/product/:id` — image gallery, size selector, add to cart, reviews
...
- About: `/about`
- Info pages: `/size-guide`, `/returns`, `/shipping`, `/privacy`, `/terms`, `/cookies`
...
- Shadow DOM: Newsletter (footer), Star Rating (product detail), Payment Fields (checkout), Contact Form
- Slots: Review cards (product detail), Cart item cards (cart slideout), Value cards (about page)
- Dynamic Locators: Load More button (home), Place Order button (checkout) — IDs regenerate per click
